...
test_linux:
	docker run -v `pwd`:`pwd` -w `pwd` --name swift-sass --rm swiftlang/swift:nightly-5.5-xenial make test

shell_linux:
	docker run -it -v `pwd`:`pwd` -w `pwd` --name swift-sass --rm swiftlang/swift:nightly-5.5-xenial /bin/bash
...
sass_embedded_version := 1.0.0-beta.12
